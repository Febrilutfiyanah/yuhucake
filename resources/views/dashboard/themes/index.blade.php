                    ...
                    <tr>
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <p class="text-gray-900">
                                @if($theme->status === 'active' || $theme->status == 1)
                        ...
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <flux:dropdown>
                                ...
                                <flux:menu>
                                    ...
                                    @if($theme->status !== 'active' && $theme->status != 1)
                                        <form id="activate-form-{{ $theme->id }}" action="{{ route('themes.activate', $theme->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <flux:menu.item icon="check-circle" onclick="event.preventDefault(); document.getElementById('activate-form-{{ $theme->id }}').submit();">
                                                Aktifkan
                                            </flux:menu.item>
                                        </form>
                                    @endif
